...
export function AdminHeader({ user, onLogout }: IAdminHeaderProps) {
  ...
  const currentMeta = navigation.find(
    (n) => n.type === 'item' && isActivePath(location.pathname, n)
  ) as Extract<NavEntry, { type: 'item' }> | undefined;

  const meta =
    currentMeta ?? (navigation[0] as Extract<NavEntry, { type: 'item' }>);
  const showCreateTestCTA =
    location.pathname === PATH.TESTS || location.pathname.startsWith('/tests/');
